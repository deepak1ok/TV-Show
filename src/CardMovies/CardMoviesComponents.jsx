...
const CardMoviesComponents = (data, mediaType) => {
  ...
  const media_type = data.data.media_type
    ...
    ? data.data.type
    : mediaType;
  const imageUrl = data.data.poster_path
    ? img_300 + data.data.poster_path
    : img_not_available;
  const title = data.data.original_title || data.data.name;
  const vote_average = parseInt(data.data.vote_average);
  const original_language = data.data.original_language;
  const release_date = data.data.release_date || data.data.first_air_date;
